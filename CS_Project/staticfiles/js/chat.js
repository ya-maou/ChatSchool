...
const renderBotMessage = async (html, modelName = "AI Model", links = [], imageLinks = [], duration) => {
    ...
    if (links && Array.isArray(links) && links.length > 0) { 
        ...
        linksButton.onclick = function() {
            toggleFloatingLinks(linksButton);
        };
        ...
        links.forEach(link => {
            console.log("Current link object:", link);

            if (typeof link === 'object' && link !== null && 'url' in link && 'title' in link) {
                const linkElement = document.createElement('a');
                linkElement.href = String(link.url);           
                linkElement.textContent = String(link.title); 
                linkElement.target = '_blank';
                linkElement.className = 'related-link-item';
                linksList.appendChild(linkElement);
            } else {
                console.error("Link object is malformed:", link);
            }
        });
    ...
    if (imageLinks && Array.isArray(imageLinks) && imageLinks.length > 0) {
        ...
        let imagesToLoad = imageLinks.length;
        const checkImagesLoaded = () => {
            imagesToLoad--;
            if (imagesToLoad === 0) {
                wrapper.appendChild(imageGallery);
                container.appendChild(wrapper);
                scrollToBottom(); // 所有圖片載入完成後再滾動
            }
        };
        ...
        imageLinks.forEach(link => {
            ...
            imageElement.alt = "來源圖片";

            imageElement.onload = () => {
                checkImagesLoaded();
            };
            imageElement.onerror = () => {
                console.error("無法載入圖片:", link);
                checkImagesLoaded(); // 即使載入失敗，也要減少計數器
            };
    ...
    if (duration !== undefined && duration !== null) {
        ...
        durationElement.classList.add("message-duration");
        durationElement.textContent = `（耗時：${duration.toFixed(2)}秒）`;
...
async function sendMessage() {
    ...
    try {
        ...
        const botResponseContent = data.response.content;
        const relatedLinks = data.response.related_links || [];
        const duration = data.response.time_seconds; 
        const usedModel = data.response.model || selectedModel;
