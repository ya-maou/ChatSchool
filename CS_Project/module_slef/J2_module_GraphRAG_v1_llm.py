...
def choose_search_strategy_with_lmstudio(question: str) -> str:
    """使用 LMStudio 判斷應該使用哪種搜尋策略"""
...
請只回答 "global" 或 "local"，不要其他說明："""
    ...
    try:
        ...
        response = llm.invoke([
            SystemMessage(content='You are a search strategy classifier. Only answer "global" or "local".'),
        ...
        if 'global' in strategy:
            ...
            # 預設使用 local（更安全）
            print(f"⚠️ LMStudio 回應不明確: {strategy}，預設使用 local")
            return 'local'
            
    except Exception as e:
        print(f"❌ LMStudio 策略判斷失敗: {e}，預設使用 local")
        return 'local'
    
# 使用 Mistral 提取實體
def extract_entities_with_lmstudio(text: str) -> List[str]:
    """使用 LMStudio 從文字中提取實體名稱"""
    ...
    try:
        ...
        response = llm.invoke([
            SystemMessage(content='You are an entity extractor. Extract entity names only, one per line.'),
            HumanMessage(content=prompt)
        ])
        ...
        print(f"🤖 LMStudio提取到實體: {filtered_entities}")
        return filtered_entities
        
    except Exception as e:
        print(f"❌ LMStudio實體提取失敗: {e}")
        return []
...
回答："""
    ...
    try:
        ...
        response = llm.invoke([
            SystemMessage(content='You are a concept extractor. Extract core and auxiliary concepts from questions.'),
            HumanMessage(content=prompt)
        ])
...
def translate_with_lmstudio(text: str, max_retries=3, delay=2) -> str:
    """使用翻譯管理器進行翻譯（映射檔案優先，然後 LMStudio）"""
...
class ContentSummaryEn0913RAG:
    ...
    def _determine_best_level_with_lmstudio(self, query: str) -> int:
        """使用 LMStudio 判斷最佳檢索層級"""
    ...
    請只回答數字 0、1 或 2，不要其他說明："""
        ...
        try:
            ...
            response = llm.invoke([
                SystemMessage(content='You are a level classifier. Only answer with a number: 0, 1, or 2.'),
                HumanMessage(content=prompt)
            ])
            
            level_str = response.content.strip()
            ...
            if match:
                ...
                print(f"🎯 LMStudio 選擇層級: Level {level}")
                return level
            else:
                print(f"⚠️ LMStudio 回應不明確: {level_str}，預設使用 Level 1")
                return 1
                
        except Exception as e:
            print(f"❌ LMStudio 層級判斷失敗: {e}，預設使用 Level 1")
            return 1
    ...
    def find_relevant_communities(self, query: str, limit: int = None, level: int = None) -> List[dict]:
        ...
        # 1. 智能選擇層級
        if level is None:
            level = self._determine_best_level_with_lmstudio(query)  # <--- 修改這裡
        else:
    ...
    def _enhance_with_triplet_graph(self, neo4j_id: str, content_text: str, article_title: str) -> dict:
        ...
        try:
            ...
            # 提取實體 - 改用 LMStudio
            entities = extract_entities_with_lmstudio(text_for_extraction)
...
def process_question(state: State):
    ...
    if is_chinese_query:
        print(f"🈶 檢測到中文查詢，準備使用翻譯管理器...")
        try:
            # 使用翻譯管理器（映射檔案優先 + LMStudio 備用）
            translated_question = translate_with_lmstudio(question)
            print(f"✅ 最終翻譯結果: {translated_question}")
        except Exception as e:
...
def retrieve(state: State):
    ...
    # 使用 LMStudio 判斷策略
    print(f"\n🤔 使用 LMStudio 判斷搜尋策略...")
    strategy = choose_search_strategy_with_lmstudio(state["original_question"])
    print(f"🎯 LMStudio 判斷結果: {strategy.upper()}")
...
Partial Answer:"""
        ...
        try:
            ...
            partial_answer = response.content.strip()
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            try:
                # === 修改這裡：正確解包 tuple ===
                answer, related_links = ask_question(question)
                ...
                if related_links and len(related_links) > 0:
                    print(f"\n{'='*60}")
                    print(f"🔗 相關連結 ({len(related_links)} 筆):")
                    print(f"{'='*60}")
                    ...
                    for i, link in enumerate(related_links, 1):
                        ...
                        if source == 'Community':
                            ...
                            print(f"      涵蓋實體: {entity_count} 個")
                            if url:
                                print(f"      網址: {url}")
                        else:
                            similarity = link.get('similarity', 0)
                            enhanced = link.get('enhanced', False)
                            graphrag_enhanced = link.get('graphrag_enhanced', False)
                            
                            status_icon = "✨" if graphrag_enhanced else ("✅" if enhanced else "⚪")
                            status_text = "圖譜增強" if graphrag_enhanced else ("增強" if enhanced else "基礎")
                            
                            print(f"\n  [{i}] {status_icon} {status_text}")
                            print(f"      標題: {title}")
