...
class TranslationManager:
    ...
    #使用映射檔案進行翻譯，未命中則使用 Mistral 進行翻譯
    ...
    def _mistral_translate(self, text: str) -> str:
        try:
            prompt = f"""請將以下中文翻譯成英文，只回傳翻譯結果：{text}"""
...
class HybridRAG:
    ...
    def weaviate_search_with_prf_adaptive(self, query: str, limit: int = 10, use_prf: bool = True) -> List[Document]:
        ...
        try:
            ...
            filtered_keywords = self._filter_keywords_with_mistral_adaptive(
                query, 
                expanded_keywords,
    ...
    def _extract_and_expand_keywords_from_graph_adaptive(self, article_urls: List[str], max_total_keywords: int = 50) -> List[str]:
        ...
        try:
            ...
            traceback.print_exc()
            return []
    def _filter_keywords_with_mistral_adaptive(self, original_query: str, candidate_keywords: List[str], target_count: int = None) -> List[str]:
        """
        動態自適應的關鍵字過濾
        
        Args:
            original_query: 原始查詢
            candidate_keywords: 候選關鍵字
            target_count: 目標數量（None 則自動計算）
        """
        try:
            # ⭐ 動態計算候選數量上限
            if len(candidate_keywords) > 100:
                ...
                sampled = (
                    candidate_keywords[:20] +  # 前 20
                    candidate_keywords[20:-10:max(1, step)] +  # 中間採樣
                    candidate_keywords[-10:]  # 後 10
                )
                candidates = list(dict.fromkeys(sampled))[:50]  # 去重並限制 50
            else:
                candidates = candidate_keywords[:30]  # 原邏輯
            ...
            if target_count is None:
                ...
                if query_words <= 5:
                    target_count = "3-5"  # 簡單問題少擴展
                elif query_words <= 10:
                    target_count = "5-8"  # 中等問題
                else:
                    target_count = "8-12"  # 複雜問題多擴展
            else:
    ...
    例如：關鍵字1, 關鍵字2, 關鍵字3"""
            ...
            print(f"🤖 [Mistral過濾] {len(candidates)} 個候選 → {len(filtered_keywords)} 個保留")
...
def infer_intent(state: State):
    ...
    try:
        ...
        # 調用 Mistral 進行意圖推理
        response = call_mistral(intent_prompt)
...
if __name__ == "__main__":
    print(f"\n{'='*70}")
    print(f"🎓 國立聯合大學 混合RAG問答系統 (簡化版)")
    print(f"{'='*70}")
    print(f"📊 系統架構: 雙路並行檢索 + 分類分析 + 結果融合")
    print(f"🛤️  路徑1: Weaviate語義檢索 → Neo4j圖譜增強")
    ...
    mistral_status = "✅ 已設定" if MISTRAL_API_KEY else "⚠️ 未設定"
    ...
    print(f"🔤 Mistral 翻譯: {mistral_status}")
