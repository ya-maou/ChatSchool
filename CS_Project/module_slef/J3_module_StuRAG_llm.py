...
# 翻譯管理器 - 支援外部翻譯檔案
# 翻譯管理器 - 支援外部翻譯檔案，使用 LMStudio 作為備用翻譯
...
def translate_with_mapping(text: str, max_retries=3, delay=2) -> str:
    """
    使用翻譯管理器進行翻譯
    1. 優先使用映射檔案
    2. 未命中則使用 LMStudio
...
def process_question(state: State):
    ...
    if is_chinese_query:
        ...
        try:
            ...
            translated_question = translate_with_mapping(question)
            ...
            try:
                ...
                if related_links:
                    ...
                    for i, link in enumerate(related_links, 1):
                        ...
                        status = "✨增強" if enhanced else "⚪基礎"
                        print(f"  [{i}] {status} | 相似度: {similarity:.3f}")
                        print(f"      {title}")
                        print(f"      {url}\n")
